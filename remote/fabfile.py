...
@task
def run_train(c, epochs=10, batch_size=64, lr=0.05, net="net50"):
    ...
    # Connect to selected GPU node
    target_conn = establish_gpu_connection(user, selected_node)
    with target_conn.prefix("source ~/miniconda3/etc/profile.d/conda.sh"):
        with target_conn.prefix("conda activate "):
            with target_conn.cd(remote_dir):
                target_conn.run(
                    f"python test.py "
                    f"--epochs {epochs} "
