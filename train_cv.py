...
#     print("optimaz: SGD and warmup cos and scheduler 0.03")
#     net.to(device)
...
#     scheduler = lrsch.SequentialLR(optimiz, schedulers=[warmup, cosine], milestones=[8])
...
def train (net, train_it, test_it, num_epoche, lernrat, device , writer, savePath):
    ...
    print('train on',device)
    print("optimaz: dropout SGD warmup ReduceLROnPlateau 0.005")
    net.to(device)
    optimiz = torch.optim.SGD(net.parameters(), lr=lernrat,  momentum=0.9)
    warmup = lrsch.LinearLR(optimiz, start_factor=0.2, total_iters=5)
    #cosine = lrsch.CosineAnnealingLR(optimiz, T_max=95, eta_min=1e-5)
    #scheduler = lrsch.SequentialLR(optimiz, schedulers=[warmup, cosine], milestones=[8])
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimiz, mode='min', factor=0.7, patience=5)
    loss = n.CrossEntropyLoss(weight=None, ignore_index=-100, reduction='mean')# with sofmax
    train_avloss, train_avacu, test_accu, test_avloss =[], [], [], []
    bestAccuracy = 0
    for epoche in tqdm(range(num_epoche)):
        ...
        if epoche < 5:
            ...
            scheduler.step(acc_los)
            print(f"Epoch {epoche}: lernrat = {optimiz.param_groups[0]['lr']:.3e}")
    return train_avloss,train_avacu,test_accu,test_avloss
'''sets setting'''
